...
function differential_experiment(res, experiment, assays, genes, species) {
  ...
  genes.forEach(g => {
    // Same guard as the baseline path: no data for this experiment -> skip, do not throw.
    // Entries here are created lazily below, so there is nothing to leave behind.
    const diffSamples = g[experiment._id];
    if (!Array.isArray(diffSamples)) return;
    diffSamples.forEach(a => {
...
async function baseline_experiment(req, res) {
  try {
    ...
    genes.forEach(g => {
      tpm[g._id] = {};
      // A gene can come back with no data for THIS experiment — the caller may have mixed
      // species (sorghum ids against an arabidopsis experiment), or the gene simply is not
      // assayed here. This used to throw on .forEach of undefined and surface as a 500.
      // Keep the empty tpm entry set just above: the rows below index tpm by REQUESTED gene
      // id, so dropping it would only move the crash. The gene gets an empty profile.
      const baselineSamples = g[experiment_id];
      if (!Array.isArray(baselineSamples)) return;
      baselineSamples.forEach(a => {
        if (a.hasOwnProperty('value') && a.value >= 0) {
